...
| Bridge console: `AUTH_FAIL` | Mod and bridge disagree on the token file (bridge restarted mid-session, or different `--data-dir`) | Restart the bridge, then leave and re-enter the match (the mod re-reads `%LOCALAPPDATA%\claude_yomih\token` when it reconnects). Use the same `--data-dir` on both stub and bridge runs. |
